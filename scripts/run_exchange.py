...
def ensure_history_tables(conn):
    conn.executescript("""
                       ...
                       (
                           ...
                           PRIMARY KEY (event_year, sender_username_norm)
                       );

                       CREATE INDEX IF NOT EXISTS idx_pairings_year_receiver
                           ON pairings_year (event_year, receiver_username_norm);

                       CREATE INDEX IF NOT EXISTS idx_pairings_all_sender
                           ON pairings_year (sender_username_norm);
...
def load_history(conn):
    # history[sender][receiver] -> list of years when that directed pairing occurred
    hist = defaultdict(lambda: defaultdict(list))
    rows = conn.execute("""
                        SELECT event_year, sender_username_norm, receiver_username_norm
                        FROM pairings_year
                        """).fetchall()

    for r in rows:
        y = int(r["event_year"])
        g = r["sender_username_norm"]
...
def score(mapping, history, current_year):
    ...
    Lower score is better. Penalize:
      - direct repeats (sender->receiver)
      - reverse repeats (receiver->sender)
    """
    total = 0.0
    for g, r in mapping.items():
...
# ---------- Persistence ----------
def write_history(conn, year, mapping):
    ...
    conn.executemany("""
                     INSERT INTO pairings_year(event_year, sender_username_norm, receiver_username_norm)
                     VALUES (?, ?, ?)
                     """, pair_rows)
...
def build_email(year, sender, receiver, subject_prefix):
    ...
    body = (
        f"Hi {sender_name},\n\n"
        f"Thanks for joining the {year} POS exchange.\n\n"
...
# TODO add rate limiter and exceptions
...
def main():
    ...
    with connect(args.db) as conn:
        ...
        # todo add report of username from -> to to paste in forum
        ...
        # todo change to write only after successfly emails sent, or write each row after each email.
        write_history(conn, args.year, mapping)
        print(f"Saved privacy-safe history for {args.year} (usernames + real names + pairings).")

        for sender_u, receiver_u in mapping.items():
            sender = people[sender_u]
            receiver = people[receiver_u]

            to_addr = (sender.get("email") or "").strip()
            if not to_addr:
                print(f"SKIP (no email for sender): username={sender_u}")
                continue

            msg = build_email(args.year, sender, receiver, args.subject_prefix)
